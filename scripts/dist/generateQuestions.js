...
async function generateQuestions(subject, topic, numQuestions) {
    ...
    for (const subject of subjects) {
        ...
        for (const topic of subject.topics) {
            ...
            try {
                ...
                console.error(`Error generating questions for ${subject.name} - ${topic}:`, error);
            }
        }
        const fileContent = `export const ${subject.name
            .toLowerCase()
            .replace(/\//g, "_")}Questions = ${JSON.stringify(questionsByTopic, null, 2)};`;
